...
oauth_states = {}
verified_users = {}  # { guild_id_str: [ {user_id, access_token, expires_at} ] }
...
# 봇 팩토리 함수
# ============================================================
def create_bot(token, bot_name, config_path, prefix):
    ...
    async def register_user(ctx, member: discord.Member):
        ...
        if member.id not in cfg["authorized_users"]:
            ...
            save_config(cfg)
            await ctx.send(f"✅ {member.mention} 님이 관리자 권한을 얻었습니다.")
        else:
    ...
    try:
        ...
        future = asyncio.run_coroutine_threadsafe(
            assign_role_from_web_wrapper(
                ...
                user_data, user_agent, access_token, expires_at
            ),
            target_bot.loop
        )
        try:
...
# ============================================================
# 웹 인증 처리 래퍼 (OAuth2 사용자 토큰으로 서버 추가)
# ============================================================
async def assign_role_from_web_wrapper(token, ip, guild_id, user_id, bot_instance, user_data, user_agent, access_token, expires_at):
    try:
        ...
        if not member:
            # OAuth2 사용자 토큰으로 서버에 추가 (공식 DM 발송)
